...
def func_ev_api(path, method):
    return sam.CfnFunction.EventSourceProperty(type="Api", properties=sam_func_api_evt(path=path, method=method))


class DdbTables(Construct):
    def __init__(self, scope: Construct, p_name_prefix: CfnParameter, id: str, table_configs, **kwargs):
...
class OauthStack(CdkStack):
    def __init__(self, scope: Construct, id: str, **kwargs):
        ...
        with open("../legacy-template-safe.yaml", "r") as f:
            CfnInclude(self, 'ExistingStack', template=yaml.load(f, Loader=yaml.SafeLoader))

        b_vault = backup.BackupVault(self, "ddb-backup-vault")
        self.backup_plan = backup.BackupPlan.daily_weekly_monthly5_year_retention(self, "backup")
        ...
        self.backup_plan.add_selection("selection", resources=list([
            backup.BackupResource.from_dynamo_db_table(table) for table in tables.tables
        ]))
        ...
        func = sam.CfnFunction(self, "oauth-func",
                               ...
                               environment=sam.CfnFunction.FunctionEnvironmentProperty(variables=dict(
