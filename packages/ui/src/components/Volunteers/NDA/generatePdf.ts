...
export const genPdf = async (
    ...
    const getMargin = (xLevel, topPadding=10) => ([xLevel * 20, topPadding, 0, 0])
    ...
    const docDefinition = {
        ...
        footer: function(currentPage, pageCount) { return { text: currentPage.toString() + ' of ' + pageCount, alignment: 'center', margin: [0, 10, 0, 0]}; },
    ...
    return {
        uri: await (new Promise((res, rej) => pdfMake.createPdf(docDefinition).getDataUrl(res))) as string
    }
}
