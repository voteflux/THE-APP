...
ddb_sse_spec = sam.CfnSimpleTable.SSESpecificationProperty
sam_func_api_evt = sam.CfnFunction.ApiEventProperty
...
def func_ev_api(path, method):
    return sam.CfnFunction.EventSourceProperty(
        type="Api", properties=sam_func_api_evt(path=path, method=method)
    )
...
class DdbTables(Construct):
    def __init__(
        self,
        scope: Construct,
        p_name_prefix: CfnParameter,
        p_stage: CfnParameter,
        id: str,
        table_configs,
        **kwargs,
    ):
        ...
        for (_t_name, _p_key) in table_configs:
            t_name = "-".join([p_name_prefix.value_as_string, p_stage.value_as_string, _t_name])
            sim_table = sam.CfnSimpleTable(
                self,
                f"{_t_name}-sim_table",
                primary_key=_p_key,
                sse_specification=ddb_sse_spec(sse_enabled=True),
                table_name=t_name,
                provisioned_throughput=None,
            )
            self.stables.append(sim_table)

            table = ddb.Table.from_table_name(
                self, f"{_t_name}-table", sim_table.table_name
            )
...
class OauthStack(CdkStack):
    def __init__(self, scope: Construct, id: str, **kwargs):
        ...
        with open("../legacy-template-safe.yaml", "r") as f:
            CfnInclude(
                self, "ExistingStack", template=yaml.load(f, Loader=yaml.SafeLoader)
            )

        b_vault = backup.BackupVault(self, "ddb-backup-vault")
        self.backup_plan = backup.BackupPlan.daily_weekly_monthly5_year_retention(
            self, "backup"
        )
        self.backup_plan.add_rule(backup.BackupPlanRule.daily(backup_vault=b_vault))
        ...
        tables = DdbTables(
            ...
            [
                ...
                ("lookups-ddb", mk_pri_key("lookup_key")),
            ],
        )
                                ...
                                resources=[
                                    ...
                                    Arn.format(
                                        ArnComponents(
                                            service="dynamodb",
                                            resource="table",
                                            resource_name=f"{p_name_prefix.value_as_string}-lookups-ddb",
                                        ),
                                        self,
                                    ),
                                ],
        ...
        func = sam.CfnFunction(
            ...
            environment=sam.CfnFunction.FunctionEnvironmentProperty(
                variables=dict(pNamePrefix=p_name_prefix.value_as_string)
            ),
