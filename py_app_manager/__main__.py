...
def _sam_pip():
    ...
    func_dirs = map(
        lambda x: f"{dir_offset}/funcs/{x}", os.listdir(f"{dir_offset}/funcs")
    )
    targets = [f"{dir_offset}/libs"] + [
        p for p in func_dirs if os.path.exists(f"{p}/requirements.txt")
    ]
    runner = CmdRunner(must_run)
    pip_install = "pip3 install -t deps -r requirements.txt --upgrade"
    for t in targets:
        runner.add(
            f"Installing python deps for {t}/requirements.txt in {t}/deps",
...
FILE_LAST_REQ_HASH = "._manager_last_pip_reqs"
...
def requirements_changed():
    ...
    try:
        with open(FILE_LAST_REQ_HASH, "r") as f:
            return curr_hash != f.read()
...
def ensure_python_deps(force=False):
    ...
    if force or (not deps_up_to_date() and not _deps_updated):
        if not requirements_changed() and not force:
            return
        must_run("python3 -m pip install -r requirements.txt")
        # must_run("ln -s sam-app/libs/flux .venv/lib/python3.7/site-packages/flux")
        last_hash = hash_file("requirements.txt")
        with open(FILE_LAST_REQ_HASH, "w") as f:
            f.write(last_hash)
    ...
    if force or (not deps_up_to_date() and not _deps_updated):
        ...
        if Repo is not None:
            ...
            if repo.is_dirty() and not force and not is_netlify():
                logging.warning(
                    "⚠️ Repository is dirty; skipping reinstall of requirements!"
                )
            else:
                logging.warning(
                    "⚠️ Detected repository but not dirty; installing requirements!"
                )
                install_deps()
        else:
            install_deps()
...
try:
    ...
    print(
        f"Please run ./manage from the root directory (of the repository) -- Exception: {e}"
    )
    sys.exit(255)

skip_ensure_deps = len(sys.argv) > 1 and (sys.argv[1] == "mgr_set_up_to_date" or False)
...
def stage_option(*args, required=True):
    def inner_stage_option(_f):
        return click.option(
            ...
            **(dict(default="dev") if required else dict(required=False)),
        )(_f)
...
def mgr_add_dep(ctx, pkgs):
    ...
    if len(pkgs) == 0:
        ...
        sys.exit(1)
    run_or(
        "python3 -m pip install %s" % (" ".join(pkgs) or "",),
        "Unable to install packages: %s" % (pkgs,),
    )
...
def install_npm_deps(target, deps, dev=False):
    logging.warning(
        "⚠️ dependencies will be installed one at a time. (It's a lerna thing)"
    )
    runner = CmdRunner(must_run)
    ...
    for d in deps:
        runner.add(
            "Installing node dependency `{d}` for `{pkg}`".format(d=d, pkg=target),
...
@cli.command()
@click.argument(
    "target", nargs=1, type=type_pkg_choice
)  # help='Package to add dependency to',
...
@cli.command()
@stage_option
@click.argument("target", nargs=1, type=click.Choice(["api", "sam", "ui", "all"]))
...
def test(stage, target, args):
    ...
    if target in {"api", "all"}:
        runner.add(
            "api tests",
            "cd packages/api && npm run test -- --stage {s} {args}".format(
                s=stage, args=" ".join(args)
            ),
        )
    if target in {"sam", "all"}:
        # runner.add("sam tests", "cd sam-app && pytest")
        runner.add("sam tests", "pytest", silent_exception=True)
    runner.run()
...
def deploy(stage, skip_tests, quick_sam, target, args):
    ...
    if target in {"api", "all"}:
        runner.add(
            "api",
            "cd packages/api && npm run deploy -- --stage {} {args}".format(
                stage, args=" ".join(args)
            ),
        )
    if target in {"sam"}:
...
def build(target, build_args, stage):
    ...
    try:
        ...
        def build_sam():
            ...
            must_run(
                f"cd sam-app && {DO_CDK_FROM_SAM} && sam build {stage_args} {rem_args}"
            )
        ...
        return (
            {"ui": build_ui, "api": build_api, "all": build_all, "sam": build_sam}[
                target
            ]
        )()
    finally:
...
@cli.command()
@click.argument(
    "dev_target", type=click.Choice(["ui", "ui-static", "api", "sam", "all"])
)
@stage_option
def dev(dev_target, stage):
    ...
    session = server.new_session(
        session_name=sess_name, start_directory="./", window_command="sleep 1"
    )
    ...
    def run_dev_cmd(dir, cmd, name, active_pane=None, vertical=False, wsl=False):
        ...
        if session is None:
            session = server.new_session(
                session_name=sess_name, start_directory=dir, window_command=to_run
            )
            # session.set_option('remain-on-exit', 'on')
            window = session.list_windows()[0]
            return window.attached_pane
        else:
            return window.split_window(
                start_directory=dir, shell=to_run, vertical=vertical
            )
    ...
    if dev_target in {"ui", "all"}:
        ui_pane = run_dev_cmd(
            "./packages/ui", "STAGE={} npm run serve".format(stage), "dev-ui"
        )

    if dev_target in {"ui-static", "all"}:
        ui_static_pane = run_dev_cmd(
            "./packages/ui", f"STAGE={stage} npm run static:serve", "dev-ui-static"
        )

    if dev_target in {"api", "all"}:
        # mongo dev server port: 53799
        mongo_pane = run_dev_cmd(
            "./packages/api", "npm run mongo-dev", "mongo-dev", vertical=False
        )
        api_cmd = "npm run watch -- --stage dev --port %d" % (api_port,)
        api_pane = run_dev_cmd(
            "./packages/api", api_cmd, "dev-api", vertical=True, active_pane=mongo_pane
        )
    ...
    if dev_target in {"sam", "all"}:
        ...
        sam_cmd = (
            f"sam local start-api --port {sam_port} --env-vars {env_file} "
            f"--host $(hostname) "
            f"--parameter-overrides ParameterKey=pNamePrefix,ParameterValue=flux-api-local-dev"
        )
        logging.info(f"sam_cmd: {sam_cmd}")
        sam_pane = run_dev_cmd(
            "./sam-app", sam_cmd, "sam-api", vertical=False, wsl=True
        )
        if stage == "dev":
            # https://github.com/tartley/rerun2
            sam_build_w_pane = run_dev_cmd(
                "./sam-app",
                "../py_app_manager/rerun2 sam build",
                "sam-build-w",
                vertical=False,
            )
